...
app.post('/api/update-product', async (req, res) => {
    try {
        ...
        await executeSql(
            `BEGIN sp_product_update(:id, :name); END;`,
            { id: prodId, name: name }
        );
...
app.post('/api/add-product', async (req, res) => {
    try {
        ...
        await executeSql(
            `BEGIN PROD_ADD_SP(:name, :desc, :img, :price, :status); END;`,
            { 
                name, 
                desc, 
                img: image, 
...
app.post('/api/calc-tax', async (req, res) => {
    try {
        ...
        const result = await executeSql(
            `BEGIN TAX_COST_SP(:st, :sub, :tax); END;`,
            { 
                st: state, 
                sub: Number(subtotal),
...
app.post('/api/ship-status', async (req, res) => {
    try {
        ...
        await executeSql(
            `BEGIN STATUS_SHIP_SP(:bid, TO_DATE(:dt, 'YYYY-MM-DD'), :shp, :trk); END;`,
            { 
                bid: Number(basketId), 
...
app.post('/api/add-basket', async (req, res) => {
    try {
        ...
        await executeSql(
            `BEGIN BASKET_ADD_SP(:bid, :pid, :price, :qty, :size, :form); END;`,
            { 
                bid: Number(basketId), 
                pid: Number(prodId), 
                price: Number(price), 
...
app.post('/api/reports/check-stock', async (req, res) => {
    ...
    try {
        ...
        const result = await executeSql(
            `BEGIN CHECK_STOCK_SP(:bid, :msg); END;`,
            { 
                bid: Number(basketId),
        ...
        console.log("Database Output:", result.outBinds.msg); // Check your VS Code terminal for this log
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
});
...
app.post('/api/reports/shopper-total', async (req, res) => {
    try {
        ...
        const result = await executeSql(
            `BEGIN :ret := TOT_PURCH_SF(:sid); END;`,
            { 
